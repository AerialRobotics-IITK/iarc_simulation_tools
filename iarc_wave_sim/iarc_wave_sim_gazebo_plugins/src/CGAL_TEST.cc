...
#include "iarc_wave_sim_gazebo_plugins/Geometry.hh"
#include "iarc_wave_sim_gazebo_plugins/Grid.hh"
#include "iarc_wave_sim_gazebo_plugins/MeshTools.hh"
#include "iarc_wave_sim_gazebo_plugins/TriangulatedGrid.hh"
...
namespace {
  ...
  std::shared_ptr<iarc::Mesh> CreateSphere(
  ...
  )
  {
    ...
    std::shared_ptr<iarc::Mesh> target = std::make_shared<iarc::Mesh>();
    iarc::MeshTools::MakeSurfaceMesh(*source, *target);
    return target;
  }
 ...
 { 
  ...
  { 
    ...
    for(auto&& face : mesh.faces()) {
      ...
      Triangle tri = iarc::Geometry::MakeTriangle(mesh, face);
...
TEST(CGAL, SurfaceMeshGrid) {
  ...
  // Intersections
  {
    ...
    // Create a sphere
    std::shared_ptr<iarc::Mesh> sphere = CreateSphere(
...
TEST(CGAL, SurfaceMeshWavefield) {
  ...
  std::shared_ptr<iarc::WaveParameters> params(new iarc::WaveParameters());
  params->SetNumber(1);
  ...
  // Wavefield
  iarc::WavefieldGerstner wavefield("TestSurfaceMeshWavefield"); 
...
TEST(CGAL, CreateTriangulationHierarchyN) {
  int N = 4;
  double L = 4.0;
  iarc::TriangulatedGrid tri_grid(N, L);
  tri_grid.CreateMesh();
