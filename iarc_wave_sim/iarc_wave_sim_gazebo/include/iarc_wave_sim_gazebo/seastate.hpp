...
public:
  ...
  double random_number(double lower_limit, double upper_limit) {
    double randNum =
        ((upper_limit - lower_limit) * ((float)rand() / RAND_MAX)) +
        lower_limit;

    return randNum;
  }

  // according to website https://wikiwaves.org/Ocean-Wave_Spectra for
  // Pierson-Moskowitz Spectrum H(1/3) = 0.22*U(10)^2/g
  ...
  std::vector<Params> v_param_ =
      make_sea_state(amplitude_, period_, total_sea_states_);
};
