...
static const std::string kConnectRosToGazeboSubtopic =
    "connect_ros_to_gazebo_subtopic";
...
template <typename T>
void model_param(const std::string &world_name, const std::string &model_name,
                 const std::string &param, T &param_value) {
  ...
  if (doc.LoadFile()) {
    ...
    if (e_param) {
      ...
      gzdbg << model_name << " model: " << dbg_param << "parameter " << param
            << " = " << param_value << " from " << doc.Value() << "\n";
    }
  }
}
} // namespace gazebo
