...
void GrabberPlugin::Load(physics::ModelPtr parent, sdf::ElementPtr sdf) {
  ...
  cmd_server_ = rosnode_->advertiseService(
      "grabber_cmd", &GrabberPlugin::serverCallback, this);

  link1_ = model_->GetLink(parent->GetName() + "::" + link_name1_);
  link2_ = model_->GetLink(parent->GetName() + "::" + link_name2_);

  updateConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GrabberPlugin::onUpdate, this, _1));

}

bool GrabberPlugin::serverCallback(
    iarc_grabber_plugin::GrabberCmd::Request &req,
