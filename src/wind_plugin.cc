...
ignition::math::Vector3d WindPlugin::LinearVel(const physics::Wind *_wind, const physics::Entity * /*_entity*/)
{
    // Compute magnitude
...
void WindPlugin::OnUpdate()
{
    ...
    for(auto const &model : models)
    {
        ...
        for(auto const &Link : links)
        {
            ...
            link->AddRelativeForce(link->GetInertial()->Mass() * this->dataPtr->forceApproximationScalingFactor * (link->RelativeWindLinearVel() - link->RelativeLinearVel()));
